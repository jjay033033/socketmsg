...
public class Client extends JFrame implements ActionListener {
	...
	public Client() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				String msg = SystemConstants.Symbol.OFFLINE + SystemConstants.Symbol.SPLIT + "全部"
						+ SystemConstants.Symbol.SPLIT + "null"
						+ SystemConstants.Symbol.SPLIT + tfdUserName.getText();
				pw.println(msg);
				pw.flush();
				System.exit(0);
			}
	...
	private void addJMenu() {
		...
		menuItemSet.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				dlg.setBounds(Client.this.getX() + 20, Client.this.getY() + 30,
						350, 150);
				...
				btnSet.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						Client.HOST = tfdHost.getText();// 先解析并判断ip是否合法
						...
						Client.PORT = Integer.parseInt(tfdPort.getText());
		...
		menuItemHelp.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				dlg.setBounds(Client.this.getX() + 30, Client.this.getY() + 30,
						400, 100);
				dlg.setLayout(new FlowLayout());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("c")) {
			if (tfdUserName.getText() == null
					...
					|| tfdUserName.getText().contains(
							SystemConstants.Symbol.SPLIT)) {
			...
			String msg = SystemConstants.Symbol.SEND
					+ SystemConstants.Symbol.SPLIT + list.getSelectedValue()
					+ SystemConstants.Symbol.SPLIT + tfdMsg.getText()
					+ SystemConstants.Symbol.SPLIT + tfdUserName.getText();
			pw.println(msg);
			pw.flush();
	...
	private void sendExitMsg() {
		String msg = SystemConstants.Symbol.OFFLINE
				+ SystemConstants.Symbol.SPLIT + "全部"
				+ SystemConstants.Symbol.SPLIT + "null"
				+ SystemConstants.Symbol.SPLIT + tfdUserName.getText();
		...
		try {
			...
			if (userName == null || userName.trim().length() == 0) {
				JOptionPane.showMessageDialog(this, "连接服务器失败!"
						+ SystemConstants.LINE_BREAK + "用户名有误，请重新输入！");
				return;
			}
		...
		public void run() {
			try {
				...
				while (sc.hasNextLine()) {
					...
					String msgs[] = str.split(SystemConstants.Symbol.SPLIT);
					System.out.println(tfdUserName.getText() + ": " + str);
					if (SystemConstants.Symbol.MSG.equals(msgs[0])) {
						if (SystemConstants.Symbol.NOTICE.equals(msgs[1])) {// 服务器发送的官方消息
							str = "[ 通知 ]:" + msgs[2];
						} else {// 服务器转发的聊天消息
							str = "[ " + msgs[1] + " ]说: " + msgs[2];
						}
						allMsg.append(SystemConstants.LINE_BREAK + str);
					}
					if (SystemConstants.Symbol.ADD.equals(msgs[0])) {
						boolean eq = false;
					...
					if (SystemConstants.Symbol.RED.equals(msgs[0])) {
